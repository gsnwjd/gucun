<template lang="html">
    <div v-if="coll_artis.length">
        <arti v-for="(arti,idx) in collartis" :key="idx" :arti="arti" />
        <button class="btn" @click="next" v-if="coll_artis.length>4">Next</button>
...
export default {
    ...
    async mounted(){
        let self = this
        let {status,data} = await self.$axios.get('http://localhost:3000/arti/getarti')

        if(status===200 && data.code===0){
            self.coll_artis = data.artis
        }
    },
    computed:{
        collartis:function(){
            ...
            return this.coll_artis.slice(str,end)
        }
    },
    methods:{
        next:function(){
            let all_len = this.coll_artis.length
