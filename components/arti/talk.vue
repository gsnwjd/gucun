...
export default {
    ...
    let {status:status2,data} = await self.$axios.get('/upload/had',{
            params:{
                id
                }
            })
    ...
    methods:{
        ...
        if(this.bg.no){
            self.$axios.post('/upload/pusharti',{id}).then(({status,data})=>{
            ...
            self.$axios.post('/upload/delarti',{id}).then(({status,data})=>{
